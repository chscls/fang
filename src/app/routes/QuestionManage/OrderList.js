...
@connect(({ rule, loading, fyOrder}) => ({
  fyOrder,
  rule,
  loading: loading.models.fyOrder,
}))
@Form.create()
export default class OrderList extends PureComponent {
  ...
  getPage = params => {
    const pagination = this.props.fyOrder.data.pagination;
  ...
  render() {
    const { fyOrder: { data }, loading } = this.props;
    ...
    const columns = [
      ...
      },
      {
        ...
        render: record => (
          <Fragment>
            <a onClick={this.modify.bind(this, record)}>支付</a>
