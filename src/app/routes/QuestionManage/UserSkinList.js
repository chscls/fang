...
@connect(({ rule, loading, fyUserSkin }) => ({
  fyUserSkin,
  rule,
  loading: loading.models.fyUserSkin,
}))
@Form.create()
export default class UserSkinList extends PureComponent {
  state = {
    modalVisible: false,
    expandForm: false,
    selectedRows: [],
  ...
  getPage = params => {
    const pagination = this.props.fyUserSkin.data.pagination;
  ...
  render() {
    const { fyUserSkin: { data }, loading } = this.props;
